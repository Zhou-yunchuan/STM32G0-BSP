...
void BSP_MPU6050_UpdateData(void)
{
	if(mpu_dmp_get_data(&mpu6050.pitch, &mpu6050.roll, &mpu6050.yaw) == 0)
	{
		...
		BSP_MPU6050_GetAccelerometer(&mpu6050.accx, &mpu6050.accy, &mpu6050.accz);	
		BSP_MPU6050_GetGyroscope(&mpu6050.gyrox, &mpu6050.gyroy, &mpu6050.gyroz);					
	}
}
...
{ 
	uint8_t res;
	BSP_I2C_Init();
	BSP_MPU6050_WriteByte(MPU_PWR_MGMT1_REG, 0X80);	// 复位MPU6050
	
  BSP_Time_DelayMs(50);
	BSP_MPU6050_WriteByte(MPU_PWR_MGMT1_REG,0X00); // 唤醒MPU6050 
	BSP_MPU6050_SetGyroFsr(3); // 陀螺仪传感器,±2000dps
	BSP_MPU6050_SetAccelFsr(0); // 加速度传感器,±2g
	BSP_MPU6050_SetRate(1000); // 设置采样率50Hz
	BSP_MPU6050_WriteByte(MPU_INT_EN_REG,0X00); // 关闭所有中断
	BSP_MPU6050_WriteByte(MPU_USER_CTRL_REG,0X00); // I2C主模式关闭
	BSP_MPU6050_WriteByte(MPU_FIFO_EN_REG,0X00); // 关闭FIFO
	BSP_MPU6050_WriteByte(MPU_INTBP_CFG_REG,0X80); // INT引脚低电平有效
	res = BSP_MPU6050_ReadByte(MPU_DEVICE_ID_REG);
	
	if(res == MPU_ADDR)
	{
		BSP_MPU6050_WriteByte(MPU_PWR_MGMT1_REG,0X01); // 设置CLKSEL,PLL X轴为参考
		BSP_MPU6050_WriteByte(MPU_PWR_MGMT2_REG,0X00); // 加速度与陀螺仪都工作
		BSP_MPU6050_SetRate(1000); // 设置采样率为50Hz
 	}
